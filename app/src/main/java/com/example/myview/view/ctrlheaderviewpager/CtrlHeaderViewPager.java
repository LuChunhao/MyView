package com.example.myview.view.ctrlheaderviewpager;
...
import com.example.myview.view.ctrlheaderviewpager.Listener.OnSlideChangeListener;


/**
 * Created by miaoyulin
 */
public class CtrlHeaderViewPager extends LinearLayout {

    private final float DAMPING = 0.6f; // 阻尼系数
    ...
    private VelocityTracker mVelocityTracker;//计算速度的工具
    ...
    private OnSlideChangeListener changeListener;

    public void setScrollChange(OnSlideChangeListener changeListener) {
    ...
    public CtrlHeaderViewPager(Context context) {
        this(context, null);
    }

    public CtrlHeaderViewPager(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CtrlHeaderViewPager(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (changeListener != null) {
                    if (deltaY > 8)
                        changeListener.upSlide();
                    else if (deltaY < -8)
                        changeListener.downSlide();
                }
            ...
            case MotionEvent.ACTION_UP:
                if (verticalScrollFlag) {
                    mVelocityTracker.computeCurrentVelocity(800, mMaximumVelocity); //1000表示单位，每1000毫秒允许滑过的最大距离是mMaximumVelocity
    ...
    public boolean canPtr() {
        ...
        try {
            if (verticalScrollFlag && mCurY == minY && mScrollable.isTop()) {
                result = true;
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public HeaderScrollHelper getmScrollable() {
        return mScrollable;
    }
